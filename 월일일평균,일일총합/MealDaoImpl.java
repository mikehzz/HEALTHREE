...
public class MealDaoImpl implements MealDao {
	...
	//월 일일평균
		@Override
		public List<MonthAvg> getMonthAvg(String memberId, String mealType) throws SQLException {
		    List<MonthAvg> statsList = new ArrayList<>();
		    ...
		    statsList = this.jdbcTemplate.query(sb.toString(), args, new RowMapper<MonthAvg>() {
		        @Override
		        public MonthAvg mapRow(ResultSet rs, int rowNum) throws SQLException {
		            MonthAvg stats = new MonthAvg();
		            stats.setMonth(rs.getString("월"));
		            stats.setAverageCalories(rs.getDouble("평균_칼로리"));
		            stats.setAverageCarbohydrates(rs.getDouble("평균_탄수화물"));
		            stats.setAverageProtein(rs.getDouble("평균_단백질"));
		            stats.setAverageFat(rs.getDouble("평균_지방"));
		            return stats;
		        }
		    });
		    return statsList;
	...
	//일일 총합
	@Override
	public List<DayTotal> getDayStats(String memberId, String mealType) {
	    List<DayTotal> statsList = new ArrayList<>();
	    StringBuilder sb = new StringBuilder(200);
	    sb.append(" SELECT \n");
	    sb.append("     M.M_DATE AS 일자, \n");
	    sb.append("     SUM(F.F_CAL) AS 일일_총_칼로리, \n");
	    sb.append("     SUM(F.F_CARBO) AS 일일_총_탄수화물, \n");
	    sb.append("     SUM(F.F_PROTEIN) AS 일일_총_단백질, \n");
	    sb.append("     SUM(F.F_FAT) AS 일일_총_지방 \n");
	    sb.append(" FROM \n");
	    sb.append("     MEAL M \n");
	    sb.append("     JOIN FOOD F ON M.F_CODE = F.F_CODE \n");
	    sb.append(" WHERE \n");
	    ...
	    // param
	    Object[] args = { memberId, mealType };
	    statsList = this.jdbcTemplate.query(sb.toString(), args, new RowMapper<DayTotal>() {
	        @Override
	        public DayTotal mapRow(ResultSet rs, int rowNum) throws SQLException {
	            DayTotal stats = new DayTotal();
	            stats.setDate(rs.getString("일자"));
	            stats.setTotalCalories(rs.getDouble("일일_총_칼로리"));
	            stats.setTotalCarbohydrates(rs.getDouble("일일_총_탄수화물"));
	            stats.setTotalProtein(rs.getDouble("일일_총_단백질"));
	            stats.setTotalFat(rs.getDouble("일일_총_지방"));
	            return stats;
	        }
	    });
	    return statsList;
	...
	@Override
	public List<MonthAvg> getMonth(String memberId, String mealType) throws SQLException {
