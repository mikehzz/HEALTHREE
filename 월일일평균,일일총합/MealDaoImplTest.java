...
public class MealDaoImplTest {
	...
	// 테스트 실행
	List<MonthAvg> statsListUser1 = dao.getMonthAvg("tester00", "B");
	assertEquals(2, statsListUser1.size());
	MonthAvg statsUser1 = statsListUser1.get(0);
	assertEquals(expectedAverageCaloriesUser1, statsUser1.getAverageCalories(), 0);
	assertEquals(expectedAverageCarbohydratesUser1, statsUser1.getAverageCarbohydrates(), 0);
	assertEquals(expectedAverageProteinUser1, statsUser1.getAverageProtein(), 0);
	assertEquals(expectedAverageFatUser1, statsUser1.getAverageFat(), 0);
	}
	...
	//일일총합
	@Test
	public void testGetDayStats() throws SQLException {
	// 예상 결과 값
	double expectedTotalCaloriesUser1 = 1463.19; // (100 + 200) + (100 + 300) = 600
	double expectedTotalCarbohydratesUser1 = 5.71; // (10 + 15) + (10 + 20) = 45
	double expectedTotalProteinUser1 = 164.44; // (5 + 7) + (5 + 10) = 22
	double expectedTotalFatUser1 = 87.15; // (2 + 3) + (2 + 5) = 10
	// 테스트 실행
	List<DayTotal> statsListUser1 = dao.getDayStats("tester00", "B");
	assertEquals(14, statsListUser1.size());
	DayTotal statsUser1 = statsListUser1.get(0);
