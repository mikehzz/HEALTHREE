...
public class PostDao {
	...
	public int add(final PostVO post) throws ClassNotFoundException, SQLException {
		...
		LOG.debug("2. param = \n" + post.toString());
		// param
		Object[] args = { post.getSeq(), post.getId(), post.getTitle(), post.getDiv(), post.getContents() };
...
//				post.getAtchFileId(),
	...
	// 게시글 조회
	public PostVO get(PostVO post) throws EmptyResultDataAccessException, ClassNotFoundException, SQLException {
		PostVO outVO = null;
		...
		sb.append("		 p_div,           \n");
		sb.append("		 p_contents,      \n");
		sb.append("		 TO_CHAR(p_reg_dt, 'YYYY-MM-DD HH24MISS') p_reg_dt,        \n");
		sb.append("		 TO_CHAR(p_mod_dt, 'YYYY-MM-DD HH24MISS') p_mod_dt,        \n");
		...
		LOG.debug("1. sql =\n" + sb.toString());
		LOG.debug("2. param = " + post.toString());

		// param
		Object[] args = { post.getSeq() };

		// jdbcTemplate
		outVO = this.jdbcTemplate.queryForObject(sb.toString(), new RowMapper<PostVO>() {
